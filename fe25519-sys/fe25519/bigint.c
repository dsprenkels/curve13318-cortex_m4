/*                          =======================
  ============================ C/C++ HEADER FILE =============================
    ...
    Taken from NaCl crypto library (Author: Matthew Dempsky, Michael Hutter, Peter Schwabe)
...
#endif
    {
        // implement the arithmetics val1 ^= temp * mask;
        // with a low and a high 16 bit word.
        // Use 32 bit operations for that, where the upper 16 bit half  
        // is filled with random data. This way, deducing the content 
        // of "condition" from the power profile is made a bit
        // more difficult, so that more power traces might be needed by an attacker.

        uint16_t randomData[8];
      
        // Empty the pointer contents first. 
        *p1 = 0;
        *p2 = 0;

        simple_randombytes((uint8_t *) randomData, 16);

        {
            uint32_t cond1 = condition | (((uint32_t) randomData[0]) << 16);
            uint32_t cond2 = condition | (((uint32_t) randomData[1]) << 16);
            uint32_t tempL = temp ^ (((uint32_t) randomData[2]) << 16);
            uint32_t tempH = (temp >> 16)    | (((uint32_t) randomData[3]) << 16);
            uint32_t val1L = val1 ^ (((uint32_t) randomData[4]) << 16);
            uint32_t val1H = (val1 >> 16)    | (((uint32_t) randomData[5]) << 16);
            uint32_t val2L = val2 ^ (((uint32_t) randomData[4]) << 16);
            uint32_t val2H = (val2 >> 16)    | (((uint32_t) randomData[5]) << 16);
            ...
            val1 = ((val1L << 16) >> 16) | (val1H << 16); 
            val2 = ((val2L << 16) >> 16) | (val2H << 16); 
        }
    }
    else
    {
        // Fixme. Randomization not yet implemented for 16 and 64 bit targets.
        val1 ^= condition * temp;
        val2 ^= condition * temp;
    }
#else
...
)
{
    ...
    for (ctr = 0; ctr < 8; ctr++)
    {
        accu += val->as_uint32_t[ctr];
        val->as_uint32_t[ctr] = (uint32_t)accu;
        accu >>= 32;
    }
}
