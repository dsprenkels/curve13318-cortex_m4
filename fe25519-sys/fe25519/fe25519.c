...
#ifndef CRYPTO_HAS_ASM_FE25519_MPY_UINT31
...
)
{
    ...
    valU31 <<= 1; // double the uint31 value.

    // First handle the most significant word.
    { 
         mpy = ((uint64_t) inOut->as_uint32_t[7]) * valU31;
         word7 = ((uint32_t) mpy) >> 1;
      
         accu = 19 * (mpy >> 32);
    }

    valU31 >>= 1; // revert the doubling.

    // Now multiply the other words.
    for (ctr = 0; ctr < 7; ctr += 1)
    {
        mpy = ((uint64_t) inOut->as_uint32_t[ctr]) * valU31;
        accu += (uint32_t) mpy;
        inOut->as_uint32_t[ctr] = (uint32_t) accu;
        accu >>= 32;
        accu += mpy >> 32;
    ...
    word7 += (uint32_t) accu;
    inOut->as_uint32_t[7] = word7;
}

#endif // ifndef CRYPTO_HAS_ASM_FE25519_MPY_UINT32
